...
test('timestamps round-trip exactly at millisecond boundaries', () => {
  for (const ms of [0, 1, 999, 1000, 59999, 60000, 3600000, 35_999_999_999])
...
test('SRT preserves BOM, CRLF, multiline text and literal markup', () => {
  const doc = parseSubtitles(
    '\uFEFF' +
      srt(undefined, undefined, '<b>Hello</b>\n你好').replaceAll('\n', '\r\n'),
  );
...
test('malformed and reversed cues fail instead of being dropped', () => {
  for (const text of [
    '',
    'not subtitles',
    srt('00:00:15,000', '00:00:05,000'),
    srt(undefined, undefined, ''),
    srt() + '\n2\nbroken\nwords',
    '\0',
  ])
  ...
  for (let a = 0; a < 20; a++)
    for (let b = a + 1; b <= 20; b++) {
      ...
      const input = {
        ...
        metadata: [],
        cues: [
          { index: 1, id: '1', start: a, end: b, text: 'x', settings: '' },
        ],
      };
      const out = applyCuts(input, cuts);
      const expected = Array.from({ length: b - a }, (_, i) => a + i).filter(
        (t) => !cuts.some((c) => c.start <= t && t < c.end),
      ).length;
      assert.equal(
        out.cues.reduce((n, c) => n + c.end - c.start, 0),
        expected,
      );
